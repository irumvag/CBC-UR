...
// ─── Level 1 config ──────────────────────────────────────────────────────────
const GRID_SIZE = 5
// 'E' = empty, 'S' = star, 'W' = wall
const GRID: ('E' | 'S' | 'W')[][] = [
  ['E', 'E', 'E', 'E', 'E'],
  ['E', 'E', 'E', 'E', 'E'],
  ['E', 'E', 'S', 'E', 'E'], // ★ at (2,2)
  ['E', 'E', 'E', 'E', 'E'],
  ['E', 'E', 'E', 'E', 'E'], // robot starts at (2,4) → NORTH
]
const START = { x: 2, y: 4 }
          ...
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="flex flex-col items-center justify-center gap-6 p-6 sm:p-10 lg:flex-1">
                    ...
                    {row.map((cell, x) => {
                      ...
                      return (
                        <div
                          key={x}
                          className={[
                            'flex h-11 w-11 items-center justify-center rounded-lg text-base font-bold transition-all duration-300 sm:h-13 sm:w-13',
                            isWall
                              ? 'bg-charcoal'
                              : 'border border-muted/20 bg-pampas-warm',
                            isRobot
                        ...
                        >
                          {isRobot && (
                            <span className="text-lg text-primary transition-transform duration-300">
                              {ARROW[robotDir]}
                            </span>
                          )}
              ...
              <div className="h-5 text-center text-sm">
                {status === 'won'     && <p className="font-semibold text-green-600">🎉 Level Complete! Well done.</p>}
            ...
            <div className="flex flex-col gap-5 p-6 sm:p-8 lg:w-72">
              ...
              <div className="border-b border-muted/20 pb-4">
                <p className="text-[10px] font-bold uppercase tracking-widest text-primary">Level 1 of 16</p>
                <h3 className="mt-1 font-bold text-foreground">Straight Line</h3>
                <p className="mt-1 text-xs leading-relaxed text-foreground/55">
                  Guide the robot (▲) to the star (★). Click command slots to build your program, then press Run.
                </p>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  <span className="inline-flex items-center gap-1 rounded-lg border border-primary/20 bg-primary/5 px-2.5 py-1.5 text-[11px] font-semibold text-primary">
                    ▲ Move Forward
                  </span>
                </div>
              </div>
                ...
                <div className="flex flex-col gap-1.5">
                  {slots.map((slot, i) => (
                    ...
                    >
                      ...
                      <span>{slot ? '▲ Move Forward' : '+ click to add'}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              {/* Hint */}
              {showHint && (
                <p className="rounded-lg border border-muted/20 bg-pampas-warm px-3 py-2 text-[11px] leading-relaxed text-foreground/60">
              ...
              <div className="flex gap-2">
                <button
                  onClick={runGame}
                  disabled={status !== 'idle'}
                  className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-primary-dark active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  ▶ Run
                </button>
                <button
                  onClick={resetGame}
                  title="Reset"
                  className="rounded-lg border border-muted/30 px-3.5 py-2.5 text-sm text-foreground/60 transition-all hover:bg-pampas-warm hover:text-foreground active:scale-95"
                >
                  ↺
                </button>
                <button
              ...
              {/* Play full version */}
              <a
                href="https://github.com/PRIEST099/logicBot"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center gap-1.5 rounded-lg border border-muted/25 py-2.5 text-sm font-medium text-foreground/60 transition-all hover:border-primary/30 hover:text-primary"
              >
                Play Full Version →
              </a>
            </div>
